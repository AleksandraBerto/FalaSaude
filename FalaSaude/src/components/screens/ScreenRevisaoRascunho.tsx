...
export const ScreenRevisaoRascunho: React.FC<ScreenRevisaoRascunhoProps> = ({
  ...
  const [activeCase, setActiveCase] = useState<'case1' | 'case2'>(
    currentPatient.caseKey === 'carlos' ? 'case1' : 'case2'
  );

  // States for unresolved pendencies in case 2
  ...
  const pendenciasRestantes = (isAllergyResolved ? 0 : 1) + (isConflictResolved ? 0 : 1);
  const isReadyToSign = activeCase === 'case1' || pendenciasRestantes === 0;

  const handleCaseSwitch = (c: 'case1' | 'case2') => {
    setActiveCase(c);
    if (c === 'case1') {
      onSelectCase('carlos');
    } else {
  ...
  const transcriptToShow = activeCase === 'case1' ? TRANSCRIPT_CARLOS : TRANSCRIPT_ANA;

  return (
    <main className="w-full pt-20 pb-24 bg-[#f8f9ff] min-h-screen">
      {/* Patient & Case Selector Top Bar */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-[#eff4ff] text-[#006a61] flex items-center justify-center font-bold text-[16px] border border-[#d3e4fe]">
              {activeCase === 'case1' ? 'CM' : 'AV'}
            </div>
            <div className="flex flex-col">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-[17px] font-bold text-[#0b1c30]">
                  {activeCase === 'case1' ? 'Carlos Eduardo Mendes' : 'Ana Maria Vasconcelos'}
                </h1>
                <span className="px-2 py-0.5 rounded bg-[#eff4ff] text-[#45464d] text-[11px] font-mono">
                  {activeCase === 'case1' ? '34 anos' : '42 anos'}
                </span>
                <span className="px-2 py-0.5 rounded bg-[#86f2e4]/30 text-[#006f66] font-mono text-[11px] font-semibold">
                  Prontuário #{activeCase === 'case1' ? '492.019/SP' : '774.209/SP'}
                </span>
              </div>
              <span className="text-[11px] font-mono text-[#76777d] mt-0.5">
                {activeCase === 'case1'
                  ? 'Entorse tornozelo direito • Box 04 • Rascunho sem bloqueios'
                  : 'Dor em Fossa Ilíaca Direita • Sala 02 • Conflito de Alergia a Dipirona'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-[#eff4ff] p-1 rounded-xl border border-[#e5eeff]">
              <button
              ...
              >
                <span>Caso 1 (Carlos - Estável)</span>
              </button>
              <button
              ...
              >
                ...
                <span>Caso 2 (Ana - Conflito Alergia)</span>
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-5 grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          {/* Active Pendencies Resolution Banner (Caso 2) */}
          {activeCase === 'case2' && (
